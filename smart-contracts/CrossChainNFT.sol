...
contract CrossChainNFT is ERC721, ERC721URIStorage, AxelarExecutable {
    ...
    constructor(
        address _gateway,
        address _gasService
    ) ERC721("CrossChainNFT", "CCNFT") AxelarExecutable(_gateway) {
    ...
    function crossChainTransfer(
        ...
        require(
            bytes(trustedRemotes[destinationChain]).length > 0,
    ...
    function _execute(
        ...
        require(
            keccak256(bytes(_sourceAddress)) ==
                keccak256(bytes(trustedRemotes[_sourceChain])),
            "Not a trusted source"
        );

        (address to, uint256 tokenId, string memory _tokenURI) = abi.decode(
            _payload,
